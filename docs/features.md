...
For most tasks in Codex, `gpt-5.5` is the recommended model. It brings the industry-leading coding capabilities of `gpt-5.4` to OpenAI’s flagship frontier model, combining frontier coding performance with stronger reasoning, native computer use, and broader professional workflows. Switch models mid-session:
