...
* Send prompts, code snippets, or screenshots (see [image inputs](#image-inputs "#image-inputs")) directly into the composer.
...
* Navigate draft history in the composer with `Up`/`Down`; Codex restores prior draft text and image markers.
...
[Remote connections](/codex/remote-connections "/codex/remote-connections"). For managed remote-control
...
[Learn more about the models available in Codex](/codex/models "/codex/models").
...
Use Codex subagent workflows to parallelize larger tasks. For setup, role configuration (`[agents]` in `config.toml`), and examples, see [Subagents](/codex/subagents "/codex/subagents").
...
The `codex cloud` command lets you triage and launch [Codex cloud tasks](/codex/cloud "/codex/cloud") without leaving the terminal. Run it with no arguments to open an interactive picker, browse active or finished tasks, and apply the changes to your local project.
...
See the [slash commands guide](/codex/guides/slash-commands "/codex/guides/slash-commands") to browse the catalog of built-ins, learn how to author custom commands, and understand where they live on disk.
...
See [Model Context Protocol](/codex/mcp "/codex/mcp") for example configurations, supported auth flows, and a more detailed guide.
